...
export const MCP_TOOLS = [
  ...
  tool("create_project", "Create an empty project + timeline", { name: str, fps: num }),
  ...
  tool("duplicate_frame", "Insert a HOLD copy after the frame", { frameId: str }, ["frameId"]),
  ...
  tool("create_tracking_point", "Drop a seed. track=true (default) runs framelab-ncc across the timeline.", { projectId: str, name: str, x: num, y: num, frameNumber: num, track: bool, provider: str }, ["projectId", "name", "x", "y", "frameNumber"]),
  ...
  tool("rerun_tracking", "Re-run NCC tracker on existing seeds. provider=locotrack → MODEL_NOT_AVAILABLE.", { timelineId: str, name: str, provider: str }, ["timelineId"]),
  ...
  tool("extract_video", "FFmpeg extract from a stored source video. High-risk.", { videoId: str, fps: num }, ["videoId"]),
  ...
  tool("suggest_breakdown_frames", "Suggest a breakdown if pose/occlusion is large. Does not create it.", { timelineId: str, startFrame: num, endFrame: num }, ["timelineId"]),
  ...
  tool("set_frame_exposure", "Set exposure_count (ones/twos/threes). Duration follows timeline fps.", { frameId: str, exposure: num, exposure_count: num }, ["frameId"]),
